// Script pour télécharger l'article complet via fetch (pas de limite)
const fs = require('fs');
...
if (!JOB_ID) {
  console.error('Usage: node download-full-article.js <jobId>');
  process.exit(1);
}

async function downloadArticle() {
  console.log('📥 Téléchargement de l\'article complet...\n');
...
${sections.map((s) => `        <li>${s.title || s.id}</li>`).join('\n')}
    </ol>
</div>
  ...
  fs.writeFileSync('ARTICLE_COMPLET_2PARTS.html', html, 'utf8');

  console.log(`\n✅ HTML généré: ARTICLE_COMPLET_2PARTS.html`);
  console.log(`📄 Taille: ${Math.round(html.length / 1024)} KB`);
  console.log(`\n🎉 Ouvrez ARTICLE_COMPLET_2PARTS.html dans votre navigateur!`);
}

downloadArticle().catch(console.error);
