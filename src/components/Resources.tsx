      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {content.resources.map((resource, index) => (
            ...
            >
              <Card className="h-full hover:shadow-lg transition-all duration-300 group cursor-pointer border-0 shadow-md overflow-hidden">
                <Link to={`/resource/${slugify(resource.title)}`}>
                  <AspectRatio ratio={16/9} className="bg-slate-100">
                    <img
                      src={`https://picsum.photos/seed/${encodeURIComponent(resource.id)}/600/338`}
                      alt={resource.title}
                      loading="lazy"
                ...
                <CardHeader>
                  ...
                  <CardTitle className="text-lg font-semibold text-slate-900 group-hover:text-teal-600 transition-colors">
                    <Link to={`/resource/${slugify(resource.title)}`}>{resource.title}</Link>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button asChild variant="ghost" size="sm" className="text-teal-600 hover:text-teal-700 p-0">
                      <Link to={resource.title.toLowerCase().includes('pharma') ? '/resource/templates-dashboard-phrama' : `/resource/${slugify(resource.title)}`}>Voir plus</Link>
                    </Button>
                  </div>
                </CardContent>
              </Card>
        ...
        >
          <Button asChild variant="outline" className="border-teal-600 text-teal-600 hover:bg-teal-50">
            <Link to="/bibliotheque"><BookOpen className="w-4 h-4 mr-2" />Voir toute la bibliothèque</Link>
          </Button>
        </motion.div>
      </div>
    </section>
